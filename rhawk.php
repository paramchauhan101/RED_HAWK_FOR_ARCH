...
if ! command -v pacman &>/dev/null; then
    echo -e "\n[!] This script is optimized for Arch-based systems. Please ensure 'pacman' is installed."
    exit 1
fi

# Install required packages if not already installed
echo -e "\nInstalling missing dependencies..."
sudo pacman -S --needed php php-curl php-xml git --noconfirm

# Verify installation
if ! php -m | grep -q 'curl'; then
    echo -e "\n[!] Failed to install php-curl. Please install it manually."
    exit 1
fi

if ! php -m | grep -q 'dom'; then
    echo -e "\n[!] Failed to install php-xml. Please install it manually."
    exit 1
fi

echo -e "\nDependencies are installed. Continuing..."
<?php
error_reporting(0);
